...
{
	BNO055_RETURN_FUNCTION_TYPE comres = BNO055_Zero_U8X;
	Wire1.beginTransmission(dev_addr);	//Start of transmission
	Wire1.write(reg_addr);				//Desired start register
	Wire1.endTransmission();	//Stop of transmission
	delayMicroseconds(150);
	Wire1.requestFrom(dev_addr, cnt);	//Request data
	while(Wire1.available())				//The slave device may send less than requested
	{
		*reg_data = Wire1.read();		//Receive a byte
...
BNO055_RETURN_FUNCTION_TYPE BNO055_I2C_bus_write(unsigned char dev_addr,unsigned char reg_addr, unsigned char *reg_data, unsigned char cnt)
{
	...
	{
		Wire1.write(*reg_data);			//Write the data
		reg_data++;						//Increment pointer
	}
	Wire1.endTransmission();	//Stop of transmission
	delayMicroseconds(150);
