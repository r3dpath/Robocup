    ...
    switch (current_state) {
        ...
        case RETURNING_BASE: {
            if (state.certainty > 1) {
                ...
                IRSensor irSensor(0xB0);  // inint

                irSensor.begin();   // setup


                irSensor.update();  //loop
                irSensor.printData();

                if (Ix[0] > 0) { // Check if it can see IR, if not rotate to find it
                    if (Ix[0] < 300) { // move to the left
                        SlowLeft();
                        SlowForward();
                    } else if (Ix[0] > 340) { // move to the right
                        SlowRight();
                        SlowForward();
                    } else { // Target is straight ahead (between 300 and 340)
                        SlowForward(); 
                    }
                } else {
                // No sources detected, rotate in a circle until weight is found    make it wall follow, 
                RightTurn();
                }
            }
            break;
